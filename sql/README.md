...
![scikit-learn](https://img.shields.io/badge/scikit--learn-KMeans%20%7C%20GMM%20%7C%20PCA-F7931E?logo=scikitlearn&logoColor=white)
...
| **K-Means без учителя** | те же 87% совпадения со скрытыми профилями (ARI 0.63, NMI 0.73) |
| **GMM** | 0.70 — проигрывает K-Means на выборке из 30 игроков, но даёт вероятности |
...
| 3. ML и EDA | `notebooks/eda_and_clustering.ipynb` | корреляции, K-Means vs GMM, PCA, динамика банкролла |
...
├── notebooks/
│   ├── eda_and_clustering.ipynb    # EDA, K-Means vs GMM, PCA, банкролл (с выводами)
...
**Подход:** 5 признаков — префлоп (`vpip_pct`, `pfr_pct`, `three_bet_pct`) и постфлоп
(`af_share` = AF/(1+AF), `wtsd_pct`) → `StandardScaler` → две модели параллельно:
`KMeans(k=4)` и `GaussianMixture(k=4)`. Число кластеров проверено локтем и silhouette;
тип ковариации GMM выбран из четырёх вариантов по BIC и осмысленности вероятностей. Номера кластеров сопоставлены со стилями венгерским алгоритмом
(`scipy.optimize.linear_sum_assignment`).

| Модель | Accuracy | ARI | NMI | Разброс по 10 seed |
|---|---|---|---|---|
| **K-Means** | **0.87** (26/30) | **0.63** | **0.73** | 0.87 → 0.87 (стабильно) |
| GMM (diag) | 0.70 | 0.49 | 0.68 | 0.63 → 0.77 |
| GMM (full, выбран по BIC) | 0.60 | 0.47 | 0.66 | — |
| SQL-правило (ручные пороги) | 0.87 | 0.66 | 0.78 | — |

**K-Means точнее и полностью воспроизводим**, GMM на 30 игроках переобучается: он оценивает
43–83 параметра по 30 наблюдениям и выделяет вырожденный кластер из одного игрока-выброса.
Постфлоп-признаки подняли NMI K-Means с 0.69 до 0.73 (Fish отделяются по AF и WTSD),
но accuracy не изменили: ошибки живут на границе TAG/LAG, а она видна только префлопом.

![Кластеры в пространстве PCA: K-Means и GMM](reports/figures/clusters_kmeans_vs_gmm.png)

При этом вероятностная модель концептуально ближе к покеру: стиль — континуум, а не ящик
(с UTG игрок выглядит как Nit, с баттона — как LAG). GMM отвечает «TAG 61% / LAG 39%»
и позволяет отправлять пограничных игроков на ручной разбор — у K-Means такой информации нет.
Чтобы это преимущество реализовалось, нужна выборка в сотни игроков.

![Вероятности GMM](reports/figures/gmm_confidence.png)
...
- **30 игроков мало для GMM**: сравнение моделей корректно только для этой выборки, на сотнях игроков вывод может смениться на противоположный.
